...
export default function EtiquetaCombiner() {
  const [etiquetas, setEtiquetas] = useState([
    { tipo: "inpost", archivo: null, texto: "Etiqueta 1" },
    { tipo: "inpost", archivo: null, texto: "Etiqueta 2" }
  ]);
  ...
  const handleAddEtiqueta = () => {
    setEtiquetas([
      ...etiquetas,
      { tipo: "inpost", archivo: null, texto: "" }
    ]);
  };
  ...
  const etiquetasConArchivo = etiquetas.filter(e => e.archivo);

  if (etiquetasConArchivo.length < 2) {
    alert("Debes añadir al menos dos etiquetas con archivo PDF para combinarlas.");
    return;
  }
  ...
  try {
    ...
    etiquetasConArchivo.forEach((etiqueta) => {
